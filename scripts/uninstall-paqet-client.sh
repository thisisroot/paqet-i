...
print_info "NOTE: Only UFW rules will be removed, not UFW itself"
echo ""
...
print_info "NOTE: This will only remove UFW rules, not uninstall UFW itself"
echo ""
...
if command -v ufw &> /dev/null; then
    ...
    if [ "$REMOVE_UFW" = "yes" ]; then
        ...
        if [ -n "$UFW_PORT" ]; then
            ...
            print_success "UFW rule removed (UFW itself remains installed)"
        else
